...
第一次只想跑通时，先按这个顺序走：`最短闭环 -> 路径 B -> 路径 B3`。`路径 B2`、`路径 C` 和 `路径 D` 分别面向已有 reviewed handoff、阶段差异对比和来源窗口 Safe Check 准备，不是首次闭环必跑步骤。
...
`examples/minimal` 用来确认 Lite 交接读法和 receiver_entry_task 语义；
`examples/context-pack-lite` 用来确认七文件 Context Pack 的阅读顺序和
`context-pack -> check -> resume -> doctor` 后续 recipe。Doctor、File-only
Export、Golden Path、Seal/Diff 和 Receiver Safe Check 都是后续分支，不是首次
smoke path 的必经步骤。

## 第一次跑通：最短闭环

目标是在 2 到 5 分钟内完成一次 local-first 闭环：

1. 让 AI 基于真实文件生成一份 Lite 交接。
2. 阅读 `readableBrief.md` 或示例 Lite 输出，确认事实、决策、风险边界和 open questions。
3. 把下一窗口开场白交给新窗口，让它先 recheck live repo state，再继续任务。
...
请用 lite 整理当前任务的短接续。
先检查真实文件，不要把推测写成事实。
请明确 verified_facts、confirmed_decisions、risk_boundaries、open_questions、receiver_entry_task 和 post_acceptance_next_action。
```

想先看一套可直接照读的公开样例，按顺序打开 [最小首次使用示例](../examples/minimal/README.md)。

预期结果：

- 交接状态是 `ready_for_receiver`。
- `verified_facts` 只包含本轮确实核对过的事实。
- `risk_boundaries` 明确哪些文件、行为或外部系统不能碰。
- `open_questions` 明确保留未知项，不把未知写成事实。
- `receiver_entry_task` 只要求下一窗口先完成入口核验。
- `post_acceptance_next_action` 才是核验之后真正继续做的项目动作。

常见失败解释：

- 缺少 `open_questions`：不是一定失败，但说明未知项没有被显式收口；补上 `not_applicable` 或真实问题。
- 把来源窗口结果写成本窗口验证：需要改成“来源窗口已验证”和“接收窗口本轮已验证”分开。
- 输出目录含 `.env`、secret 或私人绝对路径：不要继续公开使用，换到 `tests/outputs/private/` 重新生成。
- `check` 报 warning：先读 rule id。warning 通常表示需要人工判断；error 才表示产物不满足契约。
- Windows/PowerShell 里中文 Markdown 显示乱码：先按 UTF-8 重新读取，例如 `Get-Content -Encoding UTF8 <file>`，或用编辑器打开；不要把终端显示乱码直接当成文档内容错误。

## 路径 A：Skill-first Lite 交接
...
`check` 用来判断 pack 是否完整、metadata 是否可审阅、风险边界是否还在；`resume` 输出可复制的新窗口提示词。二者都不会发起 provider request。
...
## 路径 B3：Context Pack / Check / Doctor 怎么选

- `context-pack`：来源窗口打包当前 repo 的可审阅接续资料。
- `check`：接收窗口或来源窗口检查 pack 是否完整、是否过厚、是否缺少风险边界。
- `resume`：从已检查的 pack 生成下一窗口开场白。
- `doctor`：当 pack 已经 stale、broken 或和 live repo facts 不一致时，用它解释差异；不要把它当成每次都必须跑的 status 命令。
...
这两个示例分别说明 clean pack、`context-pack.too-thick` warning、broken pack，以及 `doctor.pack-head-stale` / `doctor.pack-check-error` 应该怎么判断。
