...
## Receiver Safe Check Boundary

Optional `receiver_check_config` belongs to the human-readable receiver-ready protocol. It is not a BB9 schema field and is not generated by the Handoff Builder, Adapter, Seal/Diff, or provider profile flow.

See [Receiver Safe Check](receiver-check.md) for the independent local config and result contracts.
