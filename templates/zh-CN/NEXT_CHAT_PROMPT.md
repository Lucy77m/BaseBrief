...
8. 同时保留人类可读的 `pass/fail` 结论，并等待用户确认后再继续项目动作。
9. 若 `receiver_check_config` 不是 `not_applicable`，运行固定命令：`node scripts/basebrief.js receiver-check --config <receiver_check_config> --repo <target-repo> --json`；否则继续 state-only 手动核验。
10. 如需稳定 starter 报告形状，可沿用 v1.2 Delta Receiver Report Kit 的 `current_goal`、`live_repo_state`、`inherited_fact_differences`、`hard_boundaries` 和 `next_narrow_slice`，但不要删掉本协议已有字段。
...
标准 starter 报告可以同时保留三层说明：`source-window inherited facts`、`live repo facts`、`receiver-window rechecks`。

历史 dry-run / pre-commit `commits_in_range` 数字与当前 ignored delta 不同但可解释时，若 refreshed branch、HEAD 和 worktree facts 一致，应按 non-blocking historical drift 解释，不要误判为 blocking。
...
- 单独记录实际接力摩擦。
- 先用人类可读 `pass/fail` 报告入口结论，并等待你的确认。
- 输出 `receiver_task_status`、`repository_state_status`、`declared_checks_status` 和 `handoff_acceptance`。
- 如需标准 starter report，可用 `current_goal`、`live_repo_state`、`inherited_fact_differences`、`hard_boundaries` 和 `next_narrow_slice` 组织正文，同时保留现有协议字段。
